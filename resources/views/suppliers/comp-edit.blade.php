<form action="{{ route('simcard.update',['id'=> $simcard->id]) }}" method="POST">
    @csrf
        ...
        <div class=" bg-white p-8 rounded-lg shadow-md w-5/12">
            ...
            @error('provider')
                ...
                <div >  
                    ...
                    <input type="text" name="iccid" class="block w-full  rounded-lg border-5 text-gray-900  ring-gray-300 placeholder:text-gray-400" value="{{ $simcard->iccid ?? old('iccid') }}">
                </div>

                @error('number_of_line')
                    <span class="text-red-600 mr-10">{{ $message }}</span>
                @enderror 
                <div >   
                    <label for="number_of_line" >Numero da linha *</label>         
                    <input type="text" name="number_of_line"  class="block w-full  rounded-lg border-5 text-gray-900  ring-gray-300 placeholder:text-gray-400" value="{{$simcard->number_of_line ?? old('number_of_line') }}">
                </div>
